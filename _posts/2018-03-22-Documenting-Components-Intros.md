...
*Меню (Drawer)* или *Аккордеон (Accordion)* или *Cворачиваемая секция (Collapsible)* это сложно. Еще сложнее? *Сетка (Grid)* или *макет (Layout)* или *Макет сетки (Layout Grid)*, или *Строки (Rows)* и *Колонны (Columns)*, или *Коробка (Box)*, или *Пропорциональная сетка (Proportional Grid)*, или неважно как вы называете невидимую *Разметку страницы (scaffolding)*, визуально упорядочивающию страницу. Даже *Карточка (Card)* не является универсальным определением: по сути это общий контейнер или определенной композиции из всего, что она [карточка] содержит? При создании библиотеки придется делать выбор определения.
